...
# ## OCR Running
def ocr_id_dnn(final_image, roi_id_checksheet, loaded_roi_id_checksheet):
    # Load model ML (printed)
    try:
        model = tf.keras.models.load_model(DNN_DIGIT_PRINT_PINK)
        logger.info("Model DNN successfully loaded.")
    except Exception as e:
        logger.error(f"Error loading model: {e}")
        raise e  # Re-raise the error to stop further execution
    
    print("DNN model has been loaded")
    output_folder = DNN_OUTPUT_FOLDER
    ...
    if roi_id_checksheet.is_file():
        for roi in loaded_roi_id_checksheet:
            ...
            output_path = os.path.join(output_folder, "crop_2.jpg")
            cv2.imwrite(output_path, cropped)

            # Check if the cropped region is empty
            is_filled = np.any(cropped == 0)  # Cek apakah ada isi di ROI
            if is_filled:
                cropped_processed = preprocessing_cropped_print_dnn(output_path)  # Preprocessing
                # logger.info("Roi has been cropped and preprocessed")
...
def ocr_pink_digit_print_dnn(final_image, roi_digit_print_pink, loaded_roi_digit_print_pink, target_cell_cl):
    # Load DNN Model
    try:
        model = tf.keras.models.load_model(DNN_DIGIT_PRINT_PINK)
...
def ocr_pink_handw_dnn(final_image, roi_handw, loaded_roi_handwritten, target_cell_cl_number, target_cell_cl_text):
    # Load DNN Model
    try:
        model_digit = tf.keras.models.load_model(DNN_DIGIT_HANDW_PINK)
        model_text = tf.keras.models.load_model(DNN_LETTER_PINK)
    ...
    if roi_handw.is_file():    
        ...
        for roi, target in zip(roi_number, target_cell_cl_number):
            ...
            if ratio_black > 1:
                cropped_processed = preprocessing_cropped_digit_dnn(output_path)  # Preprocessing
        ...
        for roi, target in zip(roi_letter, target_cell_cl_text):
            ...
            if ratio_black > 1:
                cropped_processed = preprocessing_cropped_letter_dnn(output_path)  # Preprocessing
                # logger.info("Roi has been cropped and preprocessed")
